...
// Or fallback to query param: /api.php?resource=users&id=1
...
// Case 1: .htaccess rewrite worked → /api.php (was /api/users)
if (preg_match('#/api/([a-z_-]+)(?:/([0-9]+))?/?$#i', $uri, $matches)) {
    $resource = strtolower($matches[1]);
    $id = isset($matches[2]) ? (int)$matches[2] : null;
} 
// Case 2: Direct call → /api.php → use query params
elseif (preg_match('#/api\.php$#i', $uri)) {
    $resource = $_GET['resource'] ?? null;
    $id = isset($_GET['id']) ? (int)$_GET['id'] : null;
}

if (!$resource) {
    respond(404, ['error' => 'No resource specified. Use /api/{resource} or /api.php?resource=...']);
}
